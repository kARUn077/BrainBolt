...
function Upload() {
    ...
    const getPdf = async () => {
        const result = await axios.get('http://localhost:4000/getFiles');
        setPdf(result.data.data[0].UploadedFiles);
    };

    const showPdf = (pdf) => {
        window.open(`http://localhost:4000/files/${pdf}`, "_blank", "noreferror");
    }

    const deletePdf = (pdf) => {
        axios.post('http://localhost:4000/deletePdf', {pdf})
        .then(result=>{
    ...
    const submitPdf = async (e) => {
        ...
        const result = await axios.post('http://localhost:4000/uploadPdf', formData, { headers: { "Content-Type": "multipart/form-data" } });
