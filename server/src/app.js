...
app.post("/createQuiz", async (req,res)=>{
    ...
    try{
        ...
        const ifExists = await StudentData.findOne({
            type:type,
        ...
        if(ifExists){
            ...
            const scheduledTime =
                req.body.quizDate + " " + req.body.quizTime;
...
app.post("/v1/quiz/next", async(req,res)=>{

    const email = req.body.email;

    const user = await StudentData.findOne({email});

    const difficulty = user.currentDifficulty || 5;

    const quiz = await StudentData.findOne({
        type:"quizDetails",
        "Query.difficulty":difficulty
    });

    if(!quiz) return res.json("No question");

    const question = quiz.Query.find(
...
//submit answer with adaptive logic
app.post("/v1/quiz/answer", async(req,res)=>{

    const {email,selected,correct} = req.body;

    const user = await StudentData.findOne({email});

    let difficulty = user.currentDifficulty || 5;
    let streak = user.streak || 0;
    let score = user.totalScore || 0;
...
//add leaderboard api
app.get("/v1/leaderboard/score", async(req,res)=>{

    const users = await StudentData
    .find({type:"student"})
    .sort({totalScore:-1})
    .limit(10);
