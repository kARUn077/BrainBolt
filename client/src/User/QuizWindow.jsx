...
function QuizWindow() {
    ...
    useEffect(() => {
        axios.post('http://localhost:4000/particularQuizDetail', { quizInfo })
            .then(result => {
   ...
   const saveQuestion = () => {
    ...
    const savedAnswers = async () => {
        const result = await axios.post('http://localhost:4000/savedAnswer', { email, quizInfo, currentQuestion, clickedOption, correctAnswer });
        showSavedAnswer();
    }

    const clearAnswers = async () => {
        const result = await axios.post('http://localhost:4000/clearAnswer', { email, quizInfo, currentQuestion, clickedOption });
        showSavedAnswer();
    }
    ...
    const showSavedAnswer = () => {
        axios.post('http://localhost:4000/result', { quizInfo, email })
            .then(result => {
    ...
    if (markedOption) {
        ...
        const found = markedOption.find(
            r => r.questionId === currentQuestion
        );

        if (found) {
            setClickedOption(found.choosen);
        } else {
            setClickedOption("");
        }
